...
import {
    TopBrowserAnalytics, TopDeviceAnalytics, TopLinks,

    TopLocationsByCity,
    TopLocationsByCountry, TopOSAnalytics, TotalClicks
} from "@/_mock/data.ts";
...
export const useAnalyticsByDevice = (device: "device" | "browser" | "OS") => {
    ...
    useEffect(() => {
        ...
        if (!(device in mockDataByDevice)) return setData({
            loading: false,
            data: null,
            error: "Invalid Device"
        });
        CreateMockData(mockDataByDevice[device]).then(r => setData({
            data: r,
            error: null,
            loading: false
        }));
...
export const useAnalyticsByLocation = (type: "country" | "city") => {
    ...
    useEffect(() => {
        setData(p => ({...p, loading: true}))
        if (!(type in mockDataByLocation)) return setData({
            loading: false,
            data: null,
            error: "Invalid Device"
        });
        CreateMockData(mockDataByLocation[type]).then(r => setData({
            data: r,
            error: null,
            loading: false
        }));
...
export const useTopLinkAnalytics = () => {
    ...
    useEffect(() => {
        setData(p => ({...p, loading: true}))
        CreateMockData(TopLinks).then(r => setData({
            data: r,
            error: null,
            loading: false
        }));
...
export const useTotalClicksAnalytics = () => {
    ...
    useEffect(() => {
        setData(p => ({...p, loading: true}))
        CreateMockData(TotalClicks).then(r => setData({
            data: r,
            error: null,
            loading: false
        }));
